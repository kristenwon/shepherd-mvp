...
class WebSocketManager:
    ...
    async def start_idle_monitor(self):
        ...
        if self._idle_check_task is None or self._idle_check_task.done():
            ...
            print(
                f"🔍 Started idle connection monitor (timeout: {self.IDLE_TIMEOUT_SECONDS}s)")
        ...
        while True:
            try:
                ...
                for run_id, idle_duration in idle_runs:
                    print(
                        f"⏰ Run {run_id[:8]} idle for {idle_duration:.0f}s, cancelling...")
                    ...
                    if self._run_manager:
                        ...
                        if success:
                            print(
                                f"✅ Successfully cancelled idle run {run_id[:8]}")
    ...
    async def _cancel_orphaned_run(self, run_id: str):
        ...
        await asyncio.sleep(600)  # Give 10 mins grace period for reconnection
        ...
        if run_id not in self._conns or not self._conns[run_id]:
            ...
            if self._run_manager:
                ...
                if success:
                    print(
                        f"✅ Successfully cancelled orphaned run {run_id[:8]}")
    ...
    async def close_all_connections(self, run_id: str, reason: str = "Run ended"):
        ...
        if run_id in self._conns:
            ...
            connections = list(self._conns[run_id])
